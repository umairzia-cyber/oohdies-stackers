...
export interface UserNFTItem {
  ...
  isCollectionQBonusActive?: boolean;
}
...
export function useContract() {
      ...
      const checkPromises = candidateIds.map(async (tokenId) => {
        try {
          ...
          if (owner.toLowerCase() === userAddress.toLowerCase()) {
            ...
            if (isAct) {
              try {
                const w = await engineContract.getWeight(tokenId);
                weightBps = Number(w);
              } catch {
                weightBps = 10000;
              }
            ...
            return {
              ...
              multiplierBps: weightBps,
              isCollectionQBonusActive: weightBps >= 20000,
            };
          }
        } catch {
          return null;
  ...
  const transferNFT = useCallback(async (tokenId: number, toAddress: string) => {
    ...
    try {
      const nftContract = new ethers.Contract(CONTRACT_ADDRESSES.OOHDIES_NFT, OOHDIES_NFT_ABI, signer);
      const ownerAddress = await signer.getAddress();
      const tx = await nftContract.safeTransferFrom(ownerAddress, toAddress, tokenId);
      setTxStatus('pending');
      setTxHash(tx.hash);
      const receipt = await tx.wait();
      ...
      throw new Error('NFT transfer transaction failed');
    } catch (err: any) {
      setTxStatus('failed');
      setTxError(err.reason || err.message || 'NFT transfer failed');
      throw err;
    } finally {
      setLoading(false);
    }
  ...
  const syncCollectionQMultiplier = useCallback(async (tokenId: number) => {
    ...
    try {
      const engineContract = new ethers.Contract(CONTRACT_ADDRESSES.EARNING_ENGINE, EARNING_ENGINE_ABI, signer);
      const tx = await engineContract.syncCollectionQ(tokenId);
      setTxStatus('pending');
      setTxHash(tx.hash);
      const receipt = await tx.wait();
      ...
      setTxStatus('failed');
      setTxError(err.reason || err.message || 'Sync failed');
      throw err;
    } finally {
      setLoading(false);
    }
